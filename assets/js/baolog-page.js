...
$(document).on('click', '#button-douyin-url', function () {
    var result = $('#input-result-url').val()
    var ct = new copy_txt();
    if (result == "") {
        $.alert('先解析再复制吧~', 30, {size: 'sm'});
    } else {
...
$(document).on('click', '#button-douyin', function () {
    ...
    $('#input-result').val("正在解析中...");
    $.get("/wp-json/balogv1/douyin", {'url': input_value}, function (res) {
        if (res.code == 200) {
            var data = res.data;
            $('#input-result-title').val(data.title);
            $('#input-result-cover').val(data.cover);
            $('#input-result-url').val(data.url);
            $.alert('OK~', 30, {size: 'sm'});
        } else {
            $('#input-result-title').val("解析失败啦，再重试一下试试~");
            $.alert('解析失败啦，再重试一下试试~', 30, {size: 'sm'});
        }
    });
});